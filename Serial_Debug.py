...
LastEditTime: 2020-08-09 15:54:59
...
class callBack():
	def sendInfo(self, Msg, res):
		...
		if res != None:
			...
			if res != False:
				...
				toMessageBox("[{}]-{}> {}".format(timeNow, res, Msg))
		else:
			toMessageBox("None!")
	...
	def sendButton_(self):
		...
		if ser.Is_open:
			...
			self.sendInfo(Msg, res)
		else:
			QMessageBox.warning(MainWindow, '警告', '串口未开启')
			connectState(0)

	def cmdEnterSend_(self, Msg):
		if ser.Is_open:
			res = ser.Send_data(Msg, lineChange=data.lineChange)
			if res != None:
				data.sendCount += res
	...
	def openSerialButton_(self):
		...
		if ser.Is_open:
			...
			connectState(2)
			if data.portname != None:
				toMessageBox('正在连接到' + data.portname)
			else:
				toMessageBox('正在连接' + ser.target)
			ser.receive_data = data.file.Load_data('Cache')
			opt = openPortThread()
			opt.exit()
			opt.openStart()
		...
		try:
			...
			if len(Msg) == 0:
				if QMessageBox.warning(MainWindow, '警告', '接收信息为空, 是否继续保存?',QMessageBox.Yes, QMessageBox.No) == QMessageBox.Yes:
					megfile = open(data.path + '/' + name, 'w', encoding = 'UTF-8')
					megfile.write(Msg)
	...
	def cleanGraphMsgButton_(self):
		global graph
		graph.graphWin.close()
		graph = MyGraphWindow(
			window.graphLayout,
			BackColor=backColorSelect(),
			antialias=data.file.Load_data('antialias'),
			showXY=data.file.Load_data('showXY'),
			showGrid=data.file.Load_data('showGrid'),
			showLegend=data.file.Load_data('showLegend'),
			showPos=data.file.Load_data('mousePos'),
			showPoint=data.file.Load_data('pointShow')
		)
...
class dataInit(QtCore.QObject):
	...
	def __init__(self, window, Back):
		...
		window.cmdToolButton.clicked.connect(Back.cmdToolButton_)
		#主页-高级设置区
		...
		#绘图
		window.cleanGraphMsgButton.clicked.connect(Back.cleanGraphMsgButton_)
...
class QMainWindowClose(QMainWindow):
	...
	def keyPressEvent(self, event):
		...
		if QApplication.keyboardModifiers() == QtCore.Qt.AltModifier:
			if event.key() == QtCore.Qt.Key_1:  #Alt + 1
				Back.sendButton1_1()
			if event.key() == QtCore.Qt.Key_2:  #Alt + 2
				Back.sendButton1_2()
			if event.key() == QtCore.Qt.Key_3:  #Alt + 3
				Back.sendButton1_3()
			if event.key() == QtCore.Qt.Key_4:  #Alt + 4
				Back.sendButton1_4()
			if event.key() == QtCore.Qt.Key_5:  #Alt + 5
				Back.sendButton1_5()
			if event.key() == QtCore.Qt.Key_6:  #Alt + 6
				Back.sendButton1_6()
			if event.key() == QtCore.Qt.Key_7:  #Alt + 7
				Back.sendButton1_7()
			if event.key() == QtCore.Qt.Key_8:  #Alt + 8
				Back.sendButton1_8()
			...
			if event.key() == QtCore.Qt.Key_S: #Alt + S  保存数据
				Back.saveMsgPushButton_()

			if event.key() == QtCore.Qt.Key_O: #Alt + O  打开/关闭串口
				Back.openSerialButton_()

			if event.key() == QtCore.Qt.Key_C: #Alt + C  清空接收
				Back.cleanMsgPushButton_()

			if event.key() == QtCore.Qt.Key_P: #Alt + P  暂停接收
				Back.stopShowButton_()

			if event.key() == QtCore.Qt.Key_R: #Alt + R  RTS
				state = window.RTS_CheckBox.checkState()
				if state == 0:
					Back.RTS_CheckBox_(1)
					window.RTS_CheckBox.setChecked(1)
				else:
					Back.RTS_CheckBox_(0)
					window.RTS_CheckBox.setChecked(0)

			if event.key() == QtCore.Qt.Key_D: #Alt + D  DTR
				state = window.DTR_CheckBox.checkState()
				if state == 0:
					Back.DTR_CheckBox_(1)
					window.DTR_CheckBox.setChecked(1)
				else:
					Back.DTR_CheckBox_(0)
...
if __name__ == '__main__':
	...
	import time
	global receiveT, graph, update, CMDLine
